...
// }
   ...
   if(request.action ==="sendFeedback"){
        if(request.vote == 8){
            makeRequest('POST',"https://www.codegrepper.com/api/feedback.php?vote=8&search_answer_result_id="+request.search_answer_result_id+"&u="+request.user_id+"&sx="+request.sx+"&sy="+request.sy+"&lx="+request.lx+"&ly="+request.ly+"&ht="+request.ht+"&tc="+request.tc,false,request.user_id,request.access_token);
        }else if(request.vote == 2){
            makeRequest('POST', "https://www.codegrepper.com/api/feedback.php?vote=2&search_answer_result_id="+request.search_answer_result_id+"&u="+request.user_id,false,request.user_id,request.access_token);
        }else if(request.vote == 5){
            makeRequest('POST', "https://www.codegrepper.com/api/feedback.php?vote=5&search_answer_result_id="+request.search_answer_result_id+"&u="+request.user_id,false,request.user_id,request.access_token);
        }else if(request.vote == 9){
            makeRequest('POST', "https://www.codegrepper.com/api/feedback.php?vote=9&search_answer_result_id="+request.search_answer_result_id+"&u="+request.user_id,false,request.user_id,request.access_token);
        }else if(request.vote == 10){
            makeRequest('POST', "https://www.codegrepper.com/api/feedback.php?vote=10&search_answer_result_id="+request.search_answer_result_id+"&u="+request.user_id,false,request.user_id,request.access_token);
        }
        sendResponse("1");
   }
});
...
function openWelcomePage(details,newUserData){
        if(newUserData.yearly_subscription_enabled && !newUserData.subscription_started_on){
         chrome.tabs.create({url: "https://www.codegrepper.com/checkout/checkout.php"}, function (tab) { });
         }else if(details.reason ==='install'){
         chrome.tabs.create({url: "https://www.codegrepper.com/welcome.php"}, function (tab) { });
        }
}
...
function runOnInstallHook2(details){
  if(details.reason == "install"){
     chrome.tabs.create({url: "https://www.codegrepper.com/app/register.php?uts=plugin"}, function (tab) { });
  }
  if(details.reason == "update"){
    //chrome.storage.sync.get([ 'grepper_user_id','userid', 'access_token', 'email','notices','shortcut_key'], function(items){
    //    console.log(items);
    //});
     //chrome.tabs.create({url: "https://www.codegrepper.com/app/update.php"}, function (tab) { });
  }
}
...
function runOnInstallHook(details){
  ...
  if(details.reason == "install" || details.reason == "update"){
        chrome.identity.getProfileUserInfo(function(info) { 
           if(info.email){
                makeRequest('GET', "https://www.codegrepper.com/api/install2.php?is_email=1&u="+info.email).then(function(data){
                ...
                getChromeUserId().then(function(data){
                    makeRequest('GET', "https://www.codegrepper.com/api/install2.php?u="+data).then(function(newData){
...
//  },5000);
 ...
 function setUserData(chromeUserID,grepperUserID,access_token,email,blacklists,hide_grepper_button,shortcut_key){
    ...
    chrome.runtime.setUninstallURL("https://www.codegrepper.com/survey.php?gid="+grepperUserID+"&cid="+chromeUserID);
...
function check_user_notifications(){
        chrome.storage.sync.get([ 'grepper_user_id','userid', 'access_token', 'email', ], function(items){
            if(items.access_token && items.grepper_user_id) {
             makeRequest('GET', "https://www.codegrepper.com/api/get_user_notifications.php",{},items.grepper_user_id,items.access_token).then(function(data){
